...
{
    printf(CYAN "ODFDocumentPut\n" RESET);
    if ((abstract->tag == HTML_HTML) && (concrete->tag == OFFICE_DOCUMENT_CONTENT)) {
        DFNode *htmlBody = DFChildWithTag(abstract,HTML_BODY);
        DFNode *ODFBody = DFChildWithTag(concrete,OFFICE_BODY);

/*        printf(CYAN "HTML BODY?\n" RESET);
        print_node_info(htmlBody);
        printf(CYAN "OFFICE_BODY?\n" RESET);
        print_node_info(ODFBody);*/

        //Don't use the Container here - stay at this level
