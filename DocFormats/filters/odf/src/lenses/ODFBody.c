...
static DFNode *ODFBodyGet(ODFGetData *get, DFNode *concrete)
{
    ...
    //elements (plus others we are not going to consider at the moment
...
{
    //here we need to switch on the document type
    //for the moment assume text
    printf(CYAN "ODFBodyPut\n" RESET);

    ODFContainerPut(put,&ODFTextLens,abstract,concrete);
}
