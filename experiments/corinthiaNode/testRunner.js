...
function copySeedDoc(doc, test) {
    fs.copySync(SEED_DIR + doc, TEST_DIR + test + "/");
}
...
exports.getTests = function() {
    ...
    for (var i = 0; i < filesArray.length; i++) {
        /*        var stat = fse.statSync(tests + filesArray[i]);
                testData.tests[i] = {};
                testData.tests[i].name = filesArray[i];
                var fdate = moment(stat.mtime);
                testData.tests[i].mtime = fdate.format("YYYY-MM-DD hh:mm:ss");*/
        //get the runtest.js file and add it to the overall JSON
        var testName = filesArray[i];
        var testObj = JSON.parse(fse.readFileSync(TEST_DIR + testName + "/" + testName + ".json", 'utf8'));
